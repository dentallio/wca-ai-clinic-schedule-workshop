...
這份 repo 保留原版牙科排班系統的視覺語言與主要操作，再拆成七個可逐步完成的技術 checkpoint。所有名稱與班表都是從零建立的合成資料；沒有登入與角色權限，**不可放入真實診所、員工、病人或 HIS 資料**。
...
開啟 `http://localhost:3000`。完整成果對應 Stage 7：
...
| Stage 5 | 顯示資料工具與本機狀態，先部署 localStorage 版本 | `checkpoint/original-ui-lesson-5-vercel-local` |
| Stage 6 | 發布 Rules、填入 Web config 並確認 Firestore 模式 | `checkpoint/original-ui-lesson-6-firestore` |
| Stage 7 | 核對 Production branch／commit 並完成三裝置同步 | `checkpoint/original-ui-lesson-7-sync` |
...
每階段講義在 `docs/stages/`；救援流程在 `COURSE_CHECKPOINTS.md`；講師流程在 `docs/INSTRUCTOR_RUNBOOK.md`。課堂第 8、9 課使用 Stage 7 成果做維護與安全判斷，不新增程式 checkpoint。
...
| `src/data/` | Stage 5～7：本機／Firestore 資料層 |
| `src/course/firebase.config.ts` | 學員自己的 Firebase Web config |

## Firebase 與部署

Stage 5 先部署 localStorage 版本。Stage 6 才依 `FIREBASE_SETUP.md` 建立學員自己的 Firebase、發布 repo 內的 `firestore.rules`，再填入 `src/course/firebase.config.ts`。Stage 7 才做三裝置同步。Firebase Web config 是前端專案識別資訊，不是資料庫門鎖；禁止提交 service account、私鑰、密碼或 token。
